...
public class Sorceress extends Player {
	...
	//2. 블리자드 스킬 - 광역공격기
	//블리자드 스킬을 마나 200을 소모
	//배열 안에 들어있는 모든 객체에 5~15 사이의 랜덤한 피해를 한번 발생을 시킵니다.
	...
	public void blizzard(Player[] players) {
		...
		if(mp < 200) {
			...
			this.mp -= 200;
			System.out.println("블리자드 기술을 사용합니다");
			
			for(Player p : players) {
			int ran = (int)(Math.random()*12)+4;
			p.hp -= ran;
			System.out.println(p.name + "가 "+ ran + "의 데미지를 입었습니다");
